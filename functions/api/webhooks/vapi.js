...
export async function onRequestPost({ request, env }) {
  ...
  let client = null;
  if (assistantId) {
    client = await env.DB.prepare('SELECT id, business_name FROM clients WHERE vapi_assistant_id = ?').bind(assistantId).first();
  }
  if (!client) {
  ...
  if (type === 'function-call' || type === 'tool-calls') {
    ...
    for (const fc of fcs.filter(Boolean)) {
      ...
      if (name === 'bookAppointment') {
        ...
        responses.push({ toolCallId: fc.id, result: 'Office has been alerted.' });
      } else {
